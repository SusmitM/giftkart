...
export const UserProfile = () => {
  ...
  const handleFormSubmit = () => {
    if (
      ...
      // Toast for empty form submission
      ...
      if (editing) {
        addressDispatch({ type: "UpdateAddressState", payload: formData });
        setShowForm(false);
        setFormData({
          Name: "",
          City: "",
         ...
         // Toast for successfully address update
      toast.success("Address Updated", {
        ...
        addressDispatch({ type: "addToAddressState", payload: formData });
        setShowForm(false);
        setEditing(false);
        setFormData({
          Name: "",
          ...
          PhoneNo: "",
            ...
            // Toast for successfully address insertion
  ...
  const deleteForm = (selectedId) => {
    addressDispatch({ type: "deleteAddressState", payload: selectedId });
          // Toast for successfully address deletion
        ...
        <ul className="headingList">
          {headings.map((heading, index) => (
            <li
              ...
              style={{
                color: selectedValue === heading ? "rgb(1, 119, 253)" : "black",
                textDecoration: selectedValue === heading ? "underline" : "",
              }}
            >
              {heading}
            </li>
          ...
          <div className="addressData">
            ...
            <ol className="addressList">
              {addressState.address.map((addressData) => (
                <li
                  className="individualAddress"
                  style={{
                    display:
                      addressData.userId === profileData._id ||
                      addressData.userId === "default"
                        ? ""
                        : "none",
                  }}
                >
                  ...
                  <div className="addressBtnContainer">
                    <button
                      className="editBtn"
                      onClick={() => editForm(addressData.id)}
                    >
                      Edit
                    </button>
                    <button
                      className="deleteBtn"
                      onClick={() => deleteForm(addressData.id)}
                    >
                      Delete
                    </button>
                  </div>
              ...
              <div className="form-wrapper">
                <form>
                  ...
                  <div className="formBtnDiv">
                    ...
                    <button type="button" onClick={handleDummyValue}>
        ...
        {selectedValue === "Orders" &&
          ...
          userData["order"][0] ? (
            <div className="orderData">
              <div className="orderData-heading">
                Total Price: ₹{userData.orderAmount}
              </div>
              <div
                className="paymentStatus"
                style={{
                  color: userData.paymentId ? "green" : "red",
                  fontSize: "1.2rem",
                  fontWeight: "600",
                }}
              >
                Payment: {userData.paymentId ? "Done" : "Not Done"}
              </div>
              <div
                className="paymentId"
                style={{ display: userData.paymentId ? "" : "none" }}
              >
              ...
              <button
                ...
                style={{ display: userData.paymentId ? "none" : "" }}
                onClick={() => makePayment(userData.orderAmount)}
              >
                Pay
              </button>
              <div
                className="paymentGuid"
                style={{ display: userData.paymentId ? "none" : "" }}
              >
                <h3>Follow the following step to make an Mock Payment.</h3>
                <ol>
                  <li>
                    Select UPI as payment mode and <b>123@sbi</b> as UPI Id.
                  </li>
                </ol>
              </div>
              <hr />
              <ul>
                {userData["order"][0].map((item) => (
                  <li>
                    <div>
                      <img
