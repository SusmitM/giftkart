...
export const PriceCard = ({page,selectedAddress}) => {
    const { cartState,TotalOriginalPrice,
        TotalCurrentPrice,
        ...
        const handelPlaceOrder=()=>{
          if(selectedAddress){
            setUserData(prev=>({...prev,order:cartState.cart,orderAmount:TotalCurrentPrice,address:[selectedAddress]}))
  ...
  return (
    <div className="cart-product-priceDetail-section">
      ...
      {page==="checkoutPage" && <div className="orderDetails">
      <h2>Order Details</h2>
      <hr />
      <div>
        <p>Item</p>
        ...
        <ol>
          {
            cartState.cart.map(({name,qty})=><li><span className="checkout-itemName">{name}</span><span className="checkout-itemQty">{qty}</span></li>)
    ...
    <div className="price-calculate-content">
      ...
      <button
        ...
        onClick={()=>page==="checkoutPage" ?handelPlaceOrder(): navigate("/checkout")}
